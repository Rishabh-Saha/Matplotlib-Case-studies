...
Dynamic models allow the agent to select different models at each step of execution based on the current context. This is useful for scenarios where different tasks may require different model capabilities or cost considerations. To use a dynamic model, you can provide a callable that returns a model instance:
...
The `Traditional LLMChain` is the simplest chain originally introduced in LangChain. This chain takes a prompt, feeds it into an LLM, and _optionally_ adds an output parsing step before returning the result.
...
Streaming is a powerful feature in LangChain that allows for real-time processing of data as it is generated by a language model (LLM). This is particularly useful for applications that require immediate feedback or interaction, such as chatbots, live content generation, and interactive storytelling.

To enable streaming in LangChain, you can set the `streaming` parameter to `True` when initializing your LLM. Additionally, you can define a `callback_manager` to handle the streaming events. The `CallbackManager` allows you to specify custom callbacks that will be triggered as new tokens are generated by the LLM.
...
result = llm.invoke("Write a poem about streaming in LangChain.")
```
This setup will stream the output of the LLM to the standard output in real-time, allowing you to see the generated content as it is produced. You can also create custom callback handlers to process the streamed data in different ways, such as updating a user interface or storing the output in a database.

## LlamaIndex

LlamaIndex is a data framework designed to facilitate the integration of external data sources with large language models (LLMs). It provides a structured approach to ingesting, indexing, and querying data, making it easier to build applications that leverage the capabilities of LLMs.
